...
import smtplib
import pyttsx3
import calGUI as dailyTasks
#import tkcalenderExamples
import Faceid
import emailReceiving
import imaplib
import email
...
def emailSending():
    ...
    def storePassword():
        ...
        if passwd.get()==passwd2.get() and (len(str(passwd.get()))!=0 and len(str(passwd2.get()))!=0 and len(str(gmail.get()))!=0):
            ...
            def send():
                try:
                    ...
                    say("Sorry, the email or the password is incorrect")
        ...
        if len(str(passwd.get()))<8 or len(str(passwd2.get()))<8:
            engine.say("Your password must contain at least 8 characters")
        else:
...
def aboutBot():
    ...
    description_label = tk.Label(root,text='''the 0py bot is a bot made by a groupe of cyber-defense students, it has functionality to allow speech recognition
facial recognition and also a calendar , it can do alot of tasks''',padx=30,pady=30,bg="White")
...
def start_thread(buttonClicked):
    if(buttonClicked == RECOGNIZE_BUTTON):#recognize speech button
        ...
        record_thread.start()
    elif(buttonClicked == AUTHENTICATE_BUTTON):#authenticate button
        record_thread = threading.Thread(target=request_recognition)
        record_thread.daemon = True
        record_thread.start()
...
def request_recognition():
    if(first_time):
        say("hi , i'm julia bot , how can i help you ?")
    else:
    ...
    elif "tasks" in recognize:
        threading.Thread(target=dailyTasks.calGUI)
    ...
    elif "check emails" in recognize and result==1:
...
about=tk.Button(root,text="About the bot",width=20,height=4,border=0,font=("helvetica",10,"bold"),bg='black',fg='white',command=aboutBot)
about.pack(pady=20)
root.mainloop()
